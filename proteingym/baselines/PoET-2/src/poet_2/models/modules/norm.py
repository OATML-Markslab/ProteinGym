...
# Modified from: https://github.com/state-spaces/mamba/blob/v2.1.0/mamba_ssm/ops/triton/layer_norm.py
# Modifications made by OpenProtein.AI under the terms of the Apache License, Version 2.0
# Modifications include:
# - Copied a subset of the original file's imports, variables, and functions.
# - Formatted the code using black and added `type: ignore` comments.
# - Simplified the RMSNorm class.
# - Added an option to disable Triton and fall back to a PyTorch RMSNorm implementation.
...
def config_prune(configs):
    if torch.version.hip:  # type: ignore
        try:
...
def _layer_norm_bwd(
    ...
    _db1 = torch.empty_like(_db) if bias1 is not None else None  # type: ignore
    ...
    db = _db.sum(0).to(bias.dtype) if bias is not None else None  # type: ignore
...
class RMSNorm(torch.nn.Module):
    ...
    def forward(
        ...
        if x.device.type == "cuda" and not disable_triton:
            return rms_norm_fn(
                x,
                self.weight,
                self.bias,
            ...
            assert residual is None and not prenorm and not residual_in_fp32
            dtype = x.dtype
...
def main():
    ...
    x = torch.rand(L, D)
    y0 = model.forward(x)
    y1 = model.forward(x, disable_triton=True)
    print(f"{(y0 - y1).abs().max().item()=:}")
